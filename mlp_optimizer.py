...
def do_optimize(nb_classes, data, labels, model_file_prefix=None, pos_class_weight=None, cold_ids=None,
    ...
    warm_indexes = np.array(warm_indexes)

    n_splits = 10
    kf = KFold(n_splits=n_splits, shuffle=True)
    sum_auc = 0
    count = 0
    for train_warm_index, test_warm_index in kf.split(warm_indexes):
        count += 1
        print("TRAIN:", train_warm_index, "TEST:", test_warm_index)
        train_index = warm_indexes[train_warm_index]
        test_index = warm_indexes[test_warm_index]
