print(setwd("H:/Python/Optimizer/R/"))
...
# pca <- prcomp(gene_go_fingerprint,scale = T)

##Compute the explained variance for different number of principal components
# pc <- cumsum(pca$sdev^2 / sum(pca$sdev^2))
# gene_descriptors_pca <- pca$x[,0:which(pc>0.999)[1]]

# rownames(gene_descriptors_pca) <- gene_list
# colnames(gene_descriptors_pca) <- seq(1,ncol(gene_descriptors_pca))
# write.csv(gene_descriptors_pca,file="data/cell_diff_gene_go_fingerprint_pca.csv", quote=FALSE)
