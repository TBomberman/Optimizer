...
def do_optimize(nb_classes, data, labels, model_file_prefix=None, class_0_weight=None, cold_ids=None, labels_float=None):
    ...
    np.savez(model_file_prefix + "_y_pred_cold", y_pred_cold)
    np.savez(model_file_prefix + "_y_true_cold_float", labels_float[cold_indexes])
    np.savez(model_file_prefix + "_y_true_cold_bin", labels[cold_indexes])
    ...
    if nb_classes > 2:
        ...
        for class_index in range(0, nb_classes):
            ...
            print_stats(train_stats, test_stats, val_stats, test_stats_cold)
    # elif nb_classes == 2:
