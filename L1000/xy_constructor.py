...
import numpy as np
import gc
from helpers import data_driven as dd
from helpers import mach_learn as ml
...
drug_features_dict = get_feature_dict('/data/datasets/mllamosa/WorkingData/smiles_rdkit_maccs.csv', use_int=True)
cell_features_dict = get_feature_dict('/data/datasets/mllamosa/WorkingData/cell_line_fingerprint.csv', use_int=True)
gene_features_dict = get_feature_dict('/data/datasets/mllamosa/WorkingData/gene_go_fingerprint.csv', use_int=True)
batch_to_drug_id_dict = get_feature_dict('/data/datasets/mllamosa/Metadata/Small_Molecule_Metadata.txt', '\t', 9)
cell_name_to_id_dict = get_feature_dict('/data/datasets/mllamosa/Metadata/Cell_Line_Metadata.txt', '\t', 0)
gene_id_dict = get_gene_id_dict()
lm_gene_entrez_ids = list(gene_id_dict.keys())[:50]
print("Loading gene expressions from gctx")
level_5_gctoo = load_gene_expression_data("/data/datasets/mllamosa/Data/GSE92742_Broad_LINCS_Level5_COMPZ.MODZ_n473647x12328.gctx", lm_gene_entrez_ids)
...
for cell_name in cell_name_to_id_dict:
    ...
    npY_class = np.reshape(npY_class,(len(npY_class),1))
    print(npX.shape, npY_class.shape)
    if npX.shape[0] > 10000:
        train_fraction = 0.7
        cut = int(len(npY_class) * train_fraction)
        ids = np.arange(len(npY_class))
        ...
        partition = dd.Partition(trainset=train_id, testset=test_id)
        model = dd.Model(data=np.hstack((npX, npY_class)),function=ml.RandF(parameters={'n_estimators': 50, 'min_samples_split': 50}), partition=partition,nfo=3)
        # model = dd.Model(data=np.hstack((data, y)),function=ml.Mlr(), partition=partition,nfo=3)
        #model = dd.Model(data=np.hstack((npX, npY_class)),function=ml.Sksvm(parameters={'regularization':1,'sigma':1}),partition=partition)
        model.training()
