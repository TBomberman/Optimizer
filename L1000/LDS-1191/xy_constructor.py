...
target_cell_names = ['PC3', 'VCAP', 'MCF7', 'A375', 'HT29', 'A549']
# target_cell_names = ['A549'] #'HT29', 'A375', 'MCF7', 'A549']
...
perc30 = int(n_maccs * 0.3)
cold_drugs_indexes = random.sample(range(0, n_maccs), perc30)
keys = list(drug_features_dict.keys())
# cold_drugs_file = open(cold_drugs_filename, 'w')
cold_drugs_keys = []
warm_drugs_keys = []
for i in range(1, n_maccs):
    key = keys[i]
    if i in cold_drugs_indexes:
        cold_drugs_keys.append(key)
    else:
        warm_drugs_keys.append(key)
            ...
            for warm_cold in ['warm', 'cold']:
                for direction in ['Both']:
                    ...
                    for i in range(length-1, -1, -1): # go backwards, assuming later experiments have stronger perturbation
                        ...
                        if warm_cold == 'warm' and drug_id not in warm_drugs_keys:
                            continue
                        if warm_cold == 'cold' and drug_id not in cold_drugs_keys:
                            continue
                        ...
                        if cell_name != target_cell_name_i:
                            continue
                    ...
                    for percentile_down in [50]:
                        model_file_prefix = data_folder_path + target_cell_name_i + '_' + direction + str(bin) + 'b_p' \
                                            + str(percentile_down) + "_" + warm_cold
                        ...
                        for cell_name in cell_name_to_id_dict:
                            ...
                            if warm_cold == 'warm':
                                warm_data = npX
                                warm_labels = npY_class
                            else:
                                cold_data = npX
                                cold_labels = npY_class

            if evaluate_type == "use_optimizer":
                do_optimize(len(np.unique(npY_class)), warm_data, warm_labels, model_file_prefix, cold_data, cold_labels)
            elif evaluate_type == "train_and_save":
                model = train_model(npX, npY_class)
                save_model(model, model_file_prefix)
